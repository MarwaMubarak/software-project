...
public class AdminController {
    ...
    public Response refundResponse(int id, int op) {
        ...
        if (op == 1) {
            Map<Integer, ServiceStatePair> current = obj.getUsersCompleteService().get(userController.getUserModel().getEmail());
            userController.getUserModel().getMyWallet().add(current.get(id).service.getCost());
            obj.getUsersCompleteService().get(userController.getUserModel().getEmail()).get(id).setState(1);
